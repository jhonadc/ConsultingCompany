// src/app/[locale]/aiservices/ai-use-policy/page.jsx
'use client'
...
/* Pills (cores) — mantém exatamente o visual anterior */
const DELIV_PILLS = [
...
const DELIV_ICONS = [
    ClipboardDocumentListIcon, // Scope & definitions
    ShieldCheckIcon,           // Permitted tools & contexts
    EyeIcon,                   // Transparency & attribution
    UserGroupIcon,             // Human oversight & escalation
    ExclamationTriangleIcon,   // Data, IP & confidentiality
    ArrowPathIcon              // Monitoring & updates
]
...
const WHY_ICONS = [
    ClipboardDocumentListIcon, // Law × Engineering
    ShieldCheckIcon,           // Defensible outcomes
    EyeIcon,                   // Precision
    UserGroupIcon,             // Enablement
    ArrowPathIcon              // Integration
]
...
export default function AiUsePolicyPage() {
    ...
    // WHY YOU NEED IT (fact strip)
    const whyNeed = ap.whyNeed || {}
    const facts = arr(whyNeed.facts)

    // DELIVERABLES
    const deliver = ap.deliverables || {}
    const deliverItems = arr(deliver.items)

    // WHY CONFORMA
    const why = ap.whyConforma || {}
    const features = arr(why.features)

    // OUTLINE
    const outline = ap.outline || {}
        ...
        <>
            ...
            <Container id="deliverables" className="mt-24 sm:mt-32">
                <SectionIntro eyebrow={deliver.eyebrow} title={deliver.title}>
                    <p className="mx-auto max-w-3xl">{deliver.body}</p>
                </SectionIntro>
            ...
            <Container id="policy-outline" className="mt-24 scroll-mt-28 sm:scroll-mt-32">
                <FadeIn className="max-w-6xl mx-auto">
                    ...
                    <div className="mt-8 space-y-4">
                        {sections.map(({ title, desc, bullets }, idx) => {
                            const colors = [
